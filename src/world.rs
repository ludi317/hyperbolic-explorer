...
/// Pillar colors, shared by both worlds so corresponding pillars look identical.
...
///
/// Generic over the symmetry group: pass the edge-reflection `generators` and a
/// `dist` from a tile center to the world origin. The hyperbolic {4,5} world and
/// the flat {4,4} world both call this, so they share ring numbering, coloring,
/// and (downstream) the pillar rule — making them correspond near the origin.
fn generate_tiles(
    generators: &[Mat4],
    max_dist: f32,
    dist: impl Fn(Vec4) -> f32,
) -> Vec<(Mat4, u32, usize)> {
...
pub fn build_world_mesh() -> Mesh {
    ...
    let tiles = generate_tiles(&edge_generators(), MAX_DIST, |c| c.w.acosh());
...
// for the split-screen comparison. It is generated by the *same* breadth-first
// edge-reflection tiler as the hyperbolic world (just with Euclidean isometries
// and four-fold instead of five-fold vertices), so the two worlds share their
// ring numbering, greedy coloring, pillar rule, and tile size. Corresponding
// tiles therefore line up pillar-for-pillar near the origin and only diverge as
// the {4,5} tiling outgrows the flat grid. Drawn with a normal Bevy camera +
// StandardMaterial so the *only* difference between the views is curvature.
// ---------------------------------------------------------------------------

/// How far out (Euclidean distance from the origin) to generate flat tiles.
const EUCLID_MAX_DIST: f32 = 15.0;

/// The four edge-reflection generators of the flat {4,4} grid — the Euclidean
/// analogue of `edge_generators`. Reflecting a tile across its k-th edge (whose
/// midpoint sits at distance `inradius` along +x) yields its neighbor there.
fn e_edge_generators(inradius: f32) -> Vec<Mat4> {
    let reflect_edge0 = Mat4::from_translation(Vec3::new(inradius, 0.0, 0.0))
        * Mat4::from_scale(Vec3::new(-1.0, 1.0, 1.0))
        * Mat4::from_translation(Vec3::new(-inradius, 0.0, 0.0));
    (0..P)
        .map(|k| {
            let a = k as f32 * 2.0 * PI / P as f32;
            Mat4::from_rotation_y(a) * reflect_edge0 * Mat4::from_rotation_y(-a)
        })
        .collect()
}
...
/// driven by the shared tiler so it mirrors the hyperbolic world tile-for-tile.
pub fn build_euclidean_mesh() -> Mesh {
    let (inradius, _) = tile_radii();
    let circumradius = inradius * SQRT_2; // flat square: corner = apothem * √2
    let tiles = generate_tiles(&e_edge_generators(inradius), EUCLID_MAX_DIST, |c| {
        c.truncate().length()
    });
    info!("generated {} euclidean tiles", tiles.len());

    let (mut pos, mut nor, mut col, mut idx) = (Vec::new(), Vec::new(), Vec::new(), Vec::new());

    for (m, ring, color) in &tiles {
        // Floor square — same corner layout as the hyperbolic tile, placed by
        // this tile's flat frame, and colored from the shared palette.
        let corners: [Vec3; 4] = std::array::from_fn(|k| {
            let a = PI / P as f32 + k as f32 * 2.0 * PI / P as f32;
            (*m * Vec4::new(circumradius * a.cos(), 0.0, circumradius * a.sin(), 1.0)).truncate()
        });
        let base = FLOOR_PALETTE[*color % FLOOR_PALETTE.len()];
        push_quad(&mut pos, &mut nor, &mut col, &mut idx, corners, Vec3::Y, base);

        // Same pillar rule as the hyperbolic world: every even ring, not origin.
        if *ring != 0 && ring % 2 == 0 {
            e_add_pillar(&mut pos, &mut nor, &mut col, &mut idx, *m);
        }
    }
...
/// Add a flat-shaded square pillar on tile `m`'s center — the Euclidean
/// analogue of `add_pillar`, sharing its footprint, height, and colors.
fn e_add_pillar(
    pos: &mut Vec<[f32; 3]>,
    nor: &mut Vec<[f32; 3]>,
    col: &mut Vec<[f32; 4]>,
    ...
    let corner =
        |a: f32, y: f32| (m * Vec4::new(PILLAR_RADIUS * a.cos(), y, PILLAR_RADIUS * a.sin(), 1.0)).truncate();
    let base: [Vec3; 4] = std::array::from_fn(|k| corner(PI / 4.0 + k as f32 * PI / 2.0, 0.0));
    let top: [Vec3; 4] =
        std::array::from_fn(|k| corner(PI / 4.0 + k as f32 * PI / 2.0, PILLAR_HEIGHT));

    // Four side walls; normal from the wall's own edges (double-sided material
    // makes the winding/sign irrelevant for shading).
    for k in 0..4 {
        let n = (k + 1) % 4;
        let quad = [base[k], base[n], top[n], top[k]];
        let normal = (quad[1] - quad[0]).cross(quad[3] - quad[0]).normalize();
        let color = if k % 2 == 0 { PILLAR_SIDE } else { PILLAR_SIDE_DARK };
        push_quad(pos, nor, col, idx, quad, normal, color);
    }
    // Top cap.
    push_quad(pos, nor, col, idx, top, Vec3::Y, PILLAR_CAP);
}
